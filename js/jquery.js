...
function jQuery(a,c) {
	...
	this.get( a.constructor == Array || a.length && !a.nodeType && a[0] != undefined && a[0].nodeType ? // 7
	...
	find: function( t, context ) {
		...
		while ( t.length > 0 && last != t ) { // 5
			...
			for ( var i = 0; i < jQuery.token.length; i += 2 ) {
				var re = new RegExp("^(" + jQuery.token[i] + ")"); //7
				var m = re.exec(t); // 8
			...
			if ( !foundToken ) {
				if ( !t.indexOf(",") || !t.indexOf("|") ) { // 9
					...
					if ( m[1] == "#" ) { // 11
						...
						var oid = document.getElementById(m[2]); //12
						r = ret = oid ? [oid] : [];  //13
						t = t.replace( re2, "" ); //14
					} else {
						if ( !m[2] || m[1] == "." ) m[2] = "*";
		...
		if ( ret && ret[0] == context ) ret.shift(); //16
		done = jQuery.merge( done, ret ); //17
